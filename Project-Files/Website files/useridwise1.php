...
if($con)
{	
	echo "<h3   style='font-family: verdana; font-size:30px;color:#88FF33;border-style: outset;text-align: center;'>UserID wise Records</h3>";
	if($username=="vsbadgujar")
	{
	$query = "SELECT batch,username,subject,room,date_time FROM id10740292_root WHERE room='302' OR room='405' ORDER BY username ASC";
	...
	{
	$query = "SELECT batch,username,subject,room,date_time FROM id10740292_root WHERE room='308' OR room='309' ORDER BY username ASC";
	$result = mysqli_query($con,$query);
	if(mysqli_num_rows($result) == 0)
	{
		echo "<h2 style='font-size: 18px;text-align: center;'>No entries found currently</h2>";
	...
	{
	$query = "SELECT batch,username,subject,room,date_time FROM id10740292_root WHERE room='303' OR room='317' ORDER BY username ASC";
	$result = mysqli_query($con,$query);
	if(mysqli_num_rows($result) == 0)
	{
		echo "<h2 style='font-size: 18px;text-align: center;'>No entries found currently</h2>";
